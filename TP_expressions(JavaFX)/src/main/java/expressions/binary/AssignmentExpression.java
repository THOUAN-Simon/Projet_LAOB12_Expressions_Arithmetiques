...
public void setParent(Expression<E> parent) throws IllegalArgumentException {
	if (parent instanceof BinaryExpression && !(parent instanceof AssignmentExpression)) {
		throw new IllegalArgumentException("AssignmentExpression<E> can't have a parent arithmetic expression");
	}
	if (parent instanceof TerminalExpression) {
		throw new IllegalArgumentException("AssignmentExpression<E> can't have a parent terminal expression");
	}
	...
	public void setLeft(Expression<E> left) throws IllegalArgumentException
	{
		if(left != null && !(left instanceof VariableExpression)) {
			throw new IllegalArgumentException("left should be a VariableExpression<E>");
		}
		super.setLeft(left);
	}
	...
	public boolean contains(Expression<E> expr) {
    	...
    	// Égalité sémantique des expressions constantes de gauche
    	if (left instanceof VariableExpression && expr instanceof VariableExpression) {
        	String leftName = ((VariableExpression<?>) left).getName();
        	String exprName = ((VariableExpression<?>) expr).getName();
        	if (leftName.equals(exprName)) return true;
    	}

    	// Égalité sémantique des expressions constantes de droite
    	if (right instanceof expressions.terminal.ConstantExpression && expr instanceof expressions.terminal.ConstantExpression) {
        	if (right.equals(expr)) return true;
    	}

    	// Commportement par défaut
    	if (left != null && left.contains(expr)) return true;
    	if (right != null && right.contains(expr)) return true;
    	return false;
	}
}
