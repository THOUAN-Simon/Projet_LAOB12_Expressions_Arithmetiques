...
public class Controller implements Initializable
{
	...
	public void initialize(URL location, ResourceBundle resources)
	{
		 ...
		 * DONE Bind Controller's specimen property to expressions model's property
		 * so that changes in the model can be reflected in this controller.
		 */
		specimen.bindBidirectional(expressionsModel.specimenProperty());

		/*
		 * Based on #contentDisplay value set during #loadPreferences
		 * DONE Set the corresponding RadioMenuItem to selected among :
		...
		else if(contentDisplay == ContentDisplay.TEXT_ONLY){
			textOnlyRadiomenuItem.setSelected(true);
		}

		/*
		 * DONE Add all buttons to be stylised to #styleableButtons
		 * 	- addButton
		 * 	- openButton
		 * 	- saveButton
		 * 	- selectAllButton
		 ...
		 * 	- quitButton
		 */
		styleableButtons.addAll(Arrays.asList(addButton, openButton, saveButton,
				selectAllButton, selectNoneButton, deleteButton, quitButton));
		...
		/*
		 * DONE Binds #expressionsTreeView's root property to model's root tree item
		 */
		expressionsTreeView.setRoot(expressionsModel.getRootItem());
	...
	public void onAddAction(ActionEvent event)
	{
		...
		try {
				expressionsModel.parse(expression);
			} catch (ParserException e) {
				Alert alert = new Alert(AlertType.INFORMATION);
				alert.setTitle("Error");
				alert.setHeaderText("Expression parsing error");
        		alert.setContentText("expression parsing error : " + e.getMessage());
				alert.showAndWait();
				messageLabel.setText("expression parsing error : the expression has not been added");
			}
		inputField.clear();
		if(expression.isEmpty())
		{
			messageLabel.setText("No expression to add, context is empty");
		}
		else
		{
			messageLabel.setText("Expression added");
		}
	}
	...
	public void onEditCommitAction(CellEditEvent<ExpressionDisplay<? extends Number>, String> event)
	{
		...
		{
			messageLabel.setText("No change in expression");
			return;
		}
	...
	public void onDeleteSelectedAction(ActionEvent event)
	{
		 ...
		 */
		List<ExpressionDisplay<Number>> selectedItems = expressionsTableView.getSelectionModel().getSelectedItems();
		 ...
		 */
		for (ExpressionDisplay<Number> expression : selectedItems)
		{
			expressionsModel.remove(expression.getExpression());
		}
	...
	public void onSaveAction(ActionEvent event)
	{
		 ...
		 */
		if (!expressionsModel.hasFileProperty().get()){
		 ...
		 */
		File file = expressionsModel.getFile();
		saveFile(file);
	}
	...
	public void onSaveAsAction(ActionEvent event)
	{
		...
		*/
    	...
    	fileChooser.setTitle("Save Expressions As");
	...
	public void onRevertAction(ActionEvent event)
	{
		...
		if (file == null)
		{
			...
			alert.setHeaderText("No file to revert to");
			alert.setContentText("No file to revert to, please open a file first");
			alert.showAndWait();
		}
		else
		{
	...
	public void onSelectNumberType(ActionEvent event)
	{
		...
		/*
		 * DONE Get all expressions model's content into a text buffer
		...
		if (source == integersRadioMenuItem)
		{
			...
			logger.info("Integer number type selected");
		}
		else if (source == floatsRadiomenuItem)
		{
			newSpecimen = Float.valueOf(0);
			logger.info("Float number type selected");
		}
		else if (source == doublesRadiomenuItem)
		{
			newSpecimen = Double.valueOf(0);
			logger.info("Double number type selected");
		}
		 ...
		 */
		if (newSpecimen != null && !newSpecimen.equals(oldSpecimen))
		{
			expressionsModel.clear();
			specimen.set(newSpecimen);
		}
		/*
		...
		try {
			...
			messageLabel.setText("Error parsing expressions: " + e.getMessage());
		}
		messageLabel.setText("Number type changed");
	}
	...
	public void onFilterAction(ActionEvent event)
	{
		...
		{
			message = "Filtering by name: " + searchField.getText();
		}
		else if (source == terminalTypeCombobox)
		{
			message = "Filtering by terminal type: " + terminalTypeCombobox.getValue();
		}
		else if (source == binaryTypeCombobox)
		{
			message = "Filtering by binary type: " + binaryTypeCombobox.getValue();
		}
		else
		{
			message = "Filtering action triggered";
		}
		logger.info(message);
	...
	public void onClearFilterAction(ActionEvent event)
	{
		...
		/*
		 * DONE Clear or reset all filtering properties
		 * 	- searchField cleared
		 ...
		 */
		searchField.clear();
		terminalTypeCombobox.getSelectionModel().selectFirst();
		binaryTypeCombobox.getSelectionModel().selectFirst();
		messageLabel.setText("Filtering cleared");
	}
	...
	public void onDisplayButtonsWithStyle(ActionEvent event)
	{
		 ...
		 */
		messageLabel.setText("Content display set to " + contentDisplay);
	}
	...
	public void onAboutAction(ActionEvent event)
	{
		...
		alert.setHeaderText("Expressions Manager v2.0");
